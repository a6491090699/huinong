...
<form method="post" id="address_form">
    ...
    <div class="bd_top_bottom-eee bg-fff padding_flanks">
        <div class="form_item">
            ...
            <input type="text" class="consignee" name="consignee" value="{{$item->name}}"  placeholder="收货人姓名"/>
        </div>
        <div class="form_item">
            <span class="font_3r color_34 goods_name-title">联系电话</span>
            <input type="text" class="consignee" name="phone_tel" value="{{$item->tel}}" placeholder="电话"/>
        </div>
        <div class="form_item">
            <span class="font_3r color_34 goods_name-title">手机</span>
            <input type="number" class="consignee" name="phone_mob" value="{{$item->phone}}"  placeholder="手机"/>
        </div>
        <div id="region"  class="form_item">
            <input type="hidden" name="region_name" value="{{$item->full_address}}" class="region_mls_names" />
            <input type="hidden" name="region_id" value="{{$item->region_id}}" class="region_mls_id" />
            <!-- <input type="hidden" name="region_name" value="河北	邯郸	大名	黄金堤乡" class="region_mls_names" />
            <input type="hidden" name="region_id" value="130425203" class="region_mls_id" /> -->
 ...
 -->
        ...
        <div class="form_item">
            ...
            <input type="text" readonly class="consignee" name="dizhi" value="{{$item->full_address}}"  placeholder=""/>
            <script type="text/javascript">
            // $(function(){
                // dizhi = $('input[name=region_name]').val();
                // $('input[name=dizhi]').val(dizhi);
        ...
        <textarea class="detailed_address" name="address" id="address" cols="30" rows="3" placeholder="地址">{{$item->street}}</textarea>
    </div>
    <div class="padding_flanks bg-fff default_setting_btn bd_top_bottom-eee">
        <div class="form_item">
            <span class="font_3r color_34 goods_name-title">设为默认</span>
            <a class="input_radio-right ">
                <input type="radio" class="input-select" name="is_default" value="1" @if($item->is_default == 1) checked @endif/>
            </a>
        </div>
    </div>
...
<script>
    ...
    $(".input_radio-right").click(function () {
        if($(this).is(".input_radio_select")){
            $(this).removeClass("input_radio_select").find("input[type='radio']").attr("checked", false);
        }else{
        ...
        $('#address_form').validate({
            ...
            submitHandler: function(form) {
                if (!submited){
                    ...
                    $.post('/member/address-save',$('#address_form').serialize(),function(d){
                        if(d.status == 'success' ){
